...
func (r *CronJob) Default() {
	...
	if r.Spec.SucessfulJobHistoryLimit == nil {
		r.Spec.SucessfulJobHistoryLimit = new(int32)
		*r.Spec.SucessfulJobHistoryLimit = 3
	}
...
func (r *CronJob) validateCronJobSpec() *field.Error {
	// the field helpers from kubernetes API machinery help us return nicely
...
func validateScheduleFormat(schedule string, fieldPath *field.Path) *field.Error {
	if _, err := cron.ParseStandard(schedule); err != nil {
		return field.Invalid(fieldPath, schedule, err.Error())
	}
	return nil
}
...
func (r *CronJob) validateCronJobName() *field.Error {
	if len(r.ObjectMeta.Name) > validationutils.DNS1035LabelMaxLength-11 {
		// The job name lenth is 63 character lika ALL K8s objects.
		// because they must fit in a DNS subdomain.
		// The CronJob controller appends a 11-character suffix to cronjob names (`-$TIMESTAMP`)
		// when creating a new job.
		// So the max job name length should be 63 - 11 = 52
		// IF we do not validate this here object creation will fail.
